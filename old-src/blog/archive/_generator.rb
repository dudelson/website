# this script generates the archives for the blog
# see http://mrloh.se/2015/06/automatic-archives-for-jekyll-on-github-pages/
# this script must be run before committing, as github pages will not do this
# part of the build for us. Right now this is implemented as a git hook
...
for tag in tags
    ...
    write_template_file(tagpage_path, "blog/tags/#{tagpath}/", tag, {tag: tag})
end
# Create template files for each year and month
for date in dates
    yearpage_path = dates_folder_path + "/#{date[:year]}.md"
    write_template_file(yearpage_path, "blog/#{date[:year]}/", date[:year], {year:"#{date[:year]}"})

    monthpage_path = dates_folder_path + "/#{date[:year]}-#{date[:month]}.md"
    month_name = "#{MONTH_NAMES[Integer(date[:month].delete('0'))]} #{date[:year]}"
    write_template_file(monthpage_path, "blog/#{date[:year]}/#{date[:month]}/", month_name, {year: date[:year], month: date[:month]})
end
